...
RSpec.describe "POST /api/v3/queries",
  ...
  context "when authorized" do
    shared_let(:project_role) { create(:project_role, permissions: %i[view_work_packages save_queries]) }
    shared_let(:member) { create(:member, user:, project:, roles: [project_role]) }
  ...
  context "when not authorized, but visible in project" do
    shared_let(:project_role) { create(:project_role, permissions: %i[view_work_packages]) }
    shared_let(:member) { create(:member, user:, project:, roles: [project_role]) }
  ...
  context "without project" do
    ...
    context "when authorized" do
      ...
      it "creates a global query" do
        ...
        expect(query).to be_present
        expect(query.project).to be_nil
        expect(query.user).to eq user
      end
    end
