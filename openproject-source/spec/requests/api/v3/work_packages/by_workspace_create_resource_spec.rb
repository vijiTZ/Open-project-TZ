...
RSpec.describe "POST api/v3/workspace/:id/work_packages", content_type: :json do
  include Rack::Test::Methods
  include API::V3::Utilities::PathHelper

  shared_let(:project) { create(:project, :with_types, public: false) }
  ...
  shared_context "with work package creation" do
    describe "notifications" do
      let(:other_user) do
        create(:user,
               member_with_permissions: { workspace => %i(view_work_packages) },
  ...
  context "for a workspace path" do
    let(:path) { api_v3_paths.work_packages_by_workspace portfolio.id }
    let(:workspace) { portfolio }
